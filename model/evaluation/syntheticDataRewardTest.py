...
class OurRLModel:
    ...
    def select_difficulty(self, state, difficulty, reward, user_embedding):
        ...
        state = torch.tensor([[state['prev_is_correct'], state['questions_roll_ct'], state['correct_answers_roll_sum'], state['percent_correct_roll'],
                               state['percent_correct_group_roll'], state['elapsed_time_cum_avg'], state['prev_is_slow'], state['cumulative_reward']]])
        user_features = torch.tensor([user_embedding])
    ...
    def retrain(self, statesDF, numInRetrainBatch, gammaVal = 0.99):
        ...
        allStatesDf = allStatesDf[['User Number', 'scaled_states', 'action', 'reward', 'Embedding List', 'game type', 'next_scaled_states', 'next Embedding List', 'next game type']]
        ...
        model_, total_loss = train(primaryModel, targetModel, dataloader, optimizer, criterion, batch_size=10, gamma=gammaVal)
        return total_loss, pd.unique(allStatesDf['User Number'])
...
def simulate_user_response(embedding_dict, user_type, difficulty, prev_state, reward_cols, rewardFunction, minMax):
...
def runExperiment(modelClass, rewardFunction, minMax, allStates, finalAvgCumulativeRewards, finalNumServedDifficulties, trainingData, minNum = 1):
    ...
    if modelClass == 'simple':
        ...
        rl_model = OurRLModel()
        gamma = int(modelClass.split(':')[1])/100
        numRetrains = 0

    stateTable = pd.DataFrame(columns = ['User Number', 'User Type', 'Embedding List', 'state', 'scaled_states', 'action', 'reward', 'reward_cols', 'embedding', 'game type'])
    ...
    # while userInfo[sessionTrackerName].min() < minNum: ## Repeats until all users have had 2 sessions
        ...
        for j in range (len(userInfo.index)):
            ...
            for i in range(10): # Mimic the 10 question session
                ...
                newState, reward, currentRewardCols, newEmbedding, scaledStates = simulate_user_response(currentEmbedding, currentUser['User Type'], currentAction, currentState, currentRewardCols, rewardFunction, minMax)
                newEmbeddingList = [newEmbedding[i]['Current Percent'] for i in range(3)]
                currentRow = [{'User Number': currentUser['User Number'], 'User Type': currentUser['User Type'], 'Embedding List': newEmbeddingList, 'state': newState, 'scaled_states': scaledStates, 'action': currentAction, 
                            'reward': reward, 'reward_cols': currentRewardCols, 'embedding': newEmbedding, 'game type': 1}]
            ...
            if 'oursRetrain' in modelClass:
                if numRetrains == 0:
                    startingData = pd.DataFrame([{'Experiment': experimentName, 'Experiment Run': experimentRun, 'Model Type': modelClass, 'Reward Type': rewardFunction,
                                    'MinMax Type': minMax, 'Num Sessions Per User': minNum, 'Gamma': gamma, 'Retrain Num': 0, 'Model Weights': primaryModel.print_model_weights()[-100:], 'Users in Training': None, 'Loss': None}])
                    trainingData = pd.concat([trainingData, startingData])
                newLoss, users_In_retrain = rl_model.retrain(stateTable, 10, gammaVal=gamma)
                numRetrains += 1
                newData = pd.DataFrame([{'Experiment': experimentName, 'Experiment Run': experimentRun, 'Model Type': modelClass, 'Reward Type': rewardFunction,
                                    'MinMax Type': minMax, 'Num Sessions Per User': minNum, 'Gamma': gamma, 'Retrain Num': numRetrains, 'Model Weights': primaryModel.print_model_weights()[-100:], 'Users in Training': users_In_retrain, 'Loss': newLoss}])
...
reward_possibilities = ['getRewards', 'constInc']

number_of_experiments = 5
min_nums = [5]
# min_nums = [1, 3, 5, 10]
filePath = 'onApr1NoRandomMinNum5NumExp5ProfBegin_'
...
allStates = pd.DataFrame(columns=['User Number', 'User Type', 'Embedding List', 'state', 'scaled_states',
       ...
       'Question Number', 'Cumultive Reward', 'Accuracy Percent',
       'Experiment Type', 'Experiment Run', 'Model Type', 'Reward Type', 'MinMax Type', 'Num Sessions Per User'])


trainingData = pd.DataFrame(columns=['Experiment', 'Experiment Run', 'Model Type', 'Reward Type','MinMax Type', 'Num Sessions Per User', 'Gamma', 'Retrain Num', 'Model Weights', 'Users in Training', 'Loss'])
        ...
        for minMaxPos in min_max_possibilites:
            for reward in reward_possibilities:
                for model in model_possibilites:
                    ...
                    allStates, not_needed, finalAvgCumulativeRewards, finalNumServedDifficulties, trainingData = runExperiment(model, reward, minMaxPos, allStates, finalAvgCumulativeRewards, finalNumServedDifficulties, trainingData, minNum = min_num)
endTime = time.time()
...
for min_num in min_nums:
    for minmax in min_max_possibilites:
        for reward in reward_possibilities:
            ...
            sns.barplot(data = finalAvgCumulativeRewards[(finalAvgCumulativeRewards['Reward Type'] == reward) & (finalAvgCumulativeRewards['MinMax Type'] == minmax) & (finalAvgCumulativeRewards['Num Sessions Per User'] == min_num)], x = "User Type", y = "Cumultive Reward", hue = "Model Type", order = userTypes)
            ...
            plt.figure(figsize=(12, 6))
            sns.barplot(data = finalAvgCumulativeRewards[(finalAvgCumulativeRewards['Reward Type'] == reward) & (finalAvgCumulativeRewards['MinMax Type'] == minmax) & (finalAvgCumulativeRewards['Num Sessions Per User'] == min_num)].groupby(['Experiment Type', 'Model Type', 'Reward Type', 'MinMax Type', 'Experiment Run', 'Num Sessions Per User'])['Cumultive Reward'].mean().reset_index(), x = "Model Type", y = "Cumultive Reward", hue = "Model Type", order = model_possibilites)
